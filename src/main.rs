...
fn main() {
    let training_data = mnist::load_mnist_file("train-images-idx3-ubyte.gz", "train-labels-idx1-ubyte.gz").unwrap();
    ...
    let mut network = MnistNetworkTrainer::new();

    network.train(training_data, testing_data, 30, 10, 3.0);
}

//The idea is that by creating the structure about a specific network layout (784 neurons into 30 into 10)
//it is more optimisable?
// ^ TODO: Make it dynamic and run benchmarks comparisons

struct MnistNetworkTrainer { //784, 30, 10
...
impl MnistNetworkTrainer {
    ...
    fn train(&mut self, mut training_data: Box<Vec<MnistImage>>, testing_data: Box<Vec<MnistImage>>, epochs: usize, batch_size: usize, eta: f64) {
    ...
    fn back_propagate(&mut self, image: &MnistImage) {
        ...
        //Begin backpropagating in final layer
        {
            ...
            self.image_d_nb[layer_index] = (activations - &image.label_array) * sigmoid_prime(weighted_inputs); //Final layer delta equation: in terms of inputted_weights and activations for the same layer
        ...
        for layer_index in (1..2).rev() {
            ...
            self.image_d_nb[layer_index] = next_weights.t().dot(next_delta) * sigmoid_prime(current_weighted_inputs); //Delta equation in terms of 'previous' delta: in terms of next weights, next delta, current weighted inputs
            self.image_d_nw[layer_index] = self.image_d_nb[layer_index].dot(&previous_activations.t()); //Nabla layer weights equation: in terms of previous layer activation and current layer delta/error. The equation on the site is never given in matrix form, but fairly logically comes down to this, including the required transposition
        }
    }
    ...
    fn feed_forward(&mut self, input_array: &Array2<f64>) {
        ...
        for layer_index in 1..3 {
            ...
            self.activation_vectors[layer_index] = sigmoid(&self.weighted_input_vectors[layer_index]);
        }

    }

    fn evaluate(&mut self, testing_data: &Box<Vec<MnistImage>>) -> f64 {
        ...
        for image in testing_data.iter() {
            ...
            let mut predicted_number: u8 = 0;
            let mut predicted_certainty: f64 = 0.;
            ...
            // println!("Actual = {}\n\nGot = {}\n\nOutput = {}\n\n", image.label, predicted_number, activation_vector);
...
#[inline]
fn sigmoid(z_vector: &Array2<f64>) -> Array2<f64> {
    //Most of the time here is in computing the negative of z_vector
    1.0 / (1.0 + (-z_vector).mapv(f64::exp))
}

#[inline]
fn sigmoid_prime(vector: &Array2<f64>) -> Array2<f64> {
    let a = sigmoid(vector);
